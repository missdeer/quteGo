...
ClientWindow::ClientWindow(QMainWindow *parent) : QMainWindow(parent), seekButtonTimer(0), oneSecondTimer(0)
{
    ...
    // connect(parser, &Parser::signal_undoRequest, qgoif,
    // &qGoIF::slot_undoRequest);
...
void ClientWindow::initStatusBar(QWidget * /*parent*/)
{
    ...
    */
    ...
    statusUsers->setWhatsThis(tr("Displays the number of current online players\nand the number of "
                                 "online players you are watching.\nA player you are watching has an "
                                 "entry in the 'watch player:' field."));
    ...
    statusGames->setWhatsThis(tr("Displays the number of games currently played on this server and the "
                                 "number of games you are observing or playing"));
    ...
    statusServer->setWhatsThis(tr("Displays the current server's name or OFFLINE "
    ...
    statusChannel->setWhatsThis(tr("Displays the current channels you are in and the number of users "
                                   "inthere.\nThe tooltip text contains the channels' title and users' "
                                   "names"));
...
void ClientWindow::timerEvent(QTimerEvent *e)
{
    ...
    {
        // qDebug(QString("%1: HoldTheLine SET: something has been
        // sent").arg(statusOnlineTime->text()));
    ...
    if (counter % 300 == 0)
    {
        ...
        if (counter == 0)
        {
            ...
            // qDebug(QString("%1 -> HoldTheLine: status =
            // %2").arg(statusOnlineTime->text()).arg(holdTheLine));
            if (holdTheLine)
            ...
            if (myAccount->num_observedgames == 0)
            {
                // nothing observing
                holdTheLine = false;
                // qDebug(QString("%1: HoldTheLine
                // END!").arg(statusOnlineTime->text()));
            }
            else
            {
                // qDebug(QString("%1: HoldTheLine LENGTHENED: observing
                // game...").arg(statusOnlineTime->text()));
            }
        }
...
void ClientWindow::sendTextToApp(const QString &txt)
{
    ...
    case SERVERNAME:
        ...
        case IGS: {
            ...
            if (1 || setting->readBoolEntry("USE_NMATCH"))
            {
                ...
                // canadian) sendcommand("nmatchrange BWN 0-9 19-19 60-60 60-3600 25-25
                // 0 0 0-0",false);
    ...
    case ACCOUNT:
        // let qgo and parser know which account in case of setting something for
        // own games
...
{
#if 0 /* @@@ Disabled for now.  Realistically, no one will use anything but \
         IGS.  */
...
void ClientWindow::slot_matchrequest(const QString &line, bool myrequest)
{
    ...
    if (myrequest)
    {
        ...
        // we want to make sure the player is selected, because the match request
        // may come from an other command (match button on the tab dialog)
...
void ClientWindow::initActions()
{
    ...
    ListView_players->setWhatsThis(tr("Table of players\n\n"
                                      ...
                                      "----\n"
                                      "toggle watch list\t\tput/remove player to/from watch list - make 'W' "
                                      "entry at 'X' column; entry/leave sounds are activated\n"
                                      "toggle exclude list\tsimilar to watch - make 'X' entry; player's "
                                      "shouts are no longer shown"));
...
{
    // connect( cb_connect, SIGNAL( activated(const QString&) ), this, SLOT(
    // slot_cbconnect(const QString&) ) );

    //  toolConnect->setText(tr("Connect with") + " " +
    //  cb_connect->currentText());
    // connect( toolConnect, SIGNAL( toggled(bool) ), this, SLOT(
    // slot_connect(bool) ) );  //end add eb 5
...
void ClientWindow::slot_seek(bool b)
{
    ...
    // if the button was just pressed on, we have already used the popup menu :
    // nothing to do
    if (b)
        return;
 ...
 * command syntax : "nmatchrange 	BWN 	0-9 19-19	 60-60
 *60-3600 	25-25 		0 0 0-0" (B/W/ nigiri)	Hcp Sz Main time (secs)
 *BY time (secs)	BY stones	Koryo time
 */
