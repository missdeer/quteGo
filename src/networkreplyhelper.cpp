...
void NetworkReplyHelper::readData(QNetworkReply *reply)
{
    ...
    auto contentEncoding = reply->rawHeader("Content-Encoding");
#if defined(NETWORK_LOG)
    qDebug() << __FUNCTION__ << __LINE__ << contentEncoding << resp.length();
#endif
    if (contentEncoding != "gzip" && contentEncoding != "deflate")
    {
        if (m_storage)
...
void *NetworkReplyHelper::rawDataPointer() const
{
    return m_rawDataPointer;
}

void NetworkReplyHelper::setData(void *data)
{
    m_rawDataPointer = data;
}
