...
/* We split the Board view into two classes: a BoardView, dealing only with
   display, and Board, which also interacts with the board window. The split is
   not 100% clean, as BoardView contains a few members that are modified only by
   the derived Board class (such as stuff related the rectangle selection), but
...
protected:
    ...
    /* It can be somewhat scary to think about how crop would interact with shifts
       and duplications.  The solution is: it does not.  Cropping is only
       allowable on fairly plain boards without shift/duplicate, such as the
       figure view, or tooltips.  */
    board_rect m_crop;
...
public:
    ...
    ~BoardView();
    /* Should be part of the constructor, but Qt doesn't seem to allow such a
     * construction with the .ui files.  */
    ...
    , public GTP_Eval_Controller
{
    ...
    /* Given to us by the main window, indicates the analyzer selected in the eval
       graph list view.  */
    analyzer_id m_an_id;
...
public:
    ...
    void external_move(game_state *st)
    {
        move_state(st);
    }
