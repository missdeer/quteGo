...
MainWindow_IGS::MainWindow_IGS(
    ...
    : MainWindow(parent, gr, nullptr, screen_key, mode), m_connector(brd)
{
...
bool qGoIF::parse_move(int src, GameInfo *gi, Game *g, QString txt)
{
    ...
    case 1:
        ...
        if (g->Sz == QString("@") || g->Sz == QString("@@"))
        {
            ...
            for (auto qb : boardlist)
            {
                if (qb->get_adj() && qb->get_id() == 10000 && qb->get_bplayer() == g->bname && qb->get_wplayer() == g->wname)
                {
    ...
    if (!qgobrd || qgobrd->get_id() != game_id)
    {
        ...
        if (!qgobrd)
        {
            ...
            if (mode == modeMatch || mode == modeTeach) // (src == 3 || src == 4 || src == 13 || src == 14
                                                        // )//|| src == 0)
            {
                if (is_own) //|| src== 0)
                {
            ...
            if (src == 0)
                qgobrd->set_Mode_real(modeObserve); // special case when not triggered by the 'observe'
                                                    // command (trail, for instance)
            else
    ...
    case 1:
        ...
        if (!qgobrd->get_havegd())
        {
            ...
            if (g->bname == myName || g->wname == myName)
            {
                ...
                // case: trying to observe own game: this is the way to get back to
                // match if board has been closed by mistake
                if (g->bname == g->wname)
                    mode = modeTeach;
                else
                    mode = modeMatch;
...
void qGoIF::slot_freegame(bool freegame)
{
    ...
    for (auto qb : boardlist)
        if (qb->get_havegd() && (qb->get_wplayer() == myName || qb->get_bplayer() == myName) && qb->get_mvcount() < 3)
        {
...
void qGoIF::slot_requestDialog(const QString &yes, const QString &no, const QString & /*id*/, const QString &opponent)
{
    ...
    else
    {
        QMessageBox mb(tr("Request of Opponent"),
                       // tr("%1 wants to %2\nYES = %3\nCANCEL =
                       // %4").arg(opp).arg(yes).arg(yes).arg(no),
                       tr("%1 wants to %2\n\nDo you accept ? \n").arg(opp).arg(yes),
...
void qGoBoard::game_startup()
{
    ...
    {
        connect(win->normalTools->btimeView, &ClockView::clicked, this, &qGoBoard::slot_addtimePauseB);
        connect(win->normalTools->wtimeView, &ClockView::clicked, this, &qGoBoard::slot_addtimePauseW);
    }
...
void qGoBoard::timerEvent(QTimerEvent *)
{
    ...
    if (mv_counter % 2)
    {
        ...
        if (chk_b < bt_i + 20)
            chk_b = bt_i;
        win->setTimes("(" + secToTime(chk_b) + ") " + secToTime(bt_i), b_stones, "(" + secToTime(chk_w) + ") " + wt, w_stones, warn, false, bt_i);
#else
    ...
    else
    {
        ...
        if (chk_w < bt_i + 20)
            chk_w = wt_i;
        win->setTimes("(" + secToTime(chk_b) + ") " + bt, b_stones, "(" + secToTime(chk_w) + ") " + secToTime(wt_i), w_stones, false, warn, wt_i);
#else
...
void qGoBoard::disconnected(bool remove_from_list)
{
    ...
    /* @@@ Sometimes we get a game result without moves, if we started observing
       just as the game ended.  We should arrange for some way to delete this
       qGoBoard.  */
}
...
void qGoBoard::send_kibitz(const QString &msg)
{
    ...
    //	qDebug() << "msg.indexOf(myName) = " <<
    // QString::number(msg.indexOf(myName)); 	if (msg.indexOf(myName) == 0) 		return;
...
void qGoBoard::slot_ttControls(bool on)
{
    ...
    if (IamTeacher)
    {
        // check whether controls key has been clicked or has been toggled by
        // command
