...
/* Handle a click on the Done button.  Return true if we should return to normal
 * mode.  */
...
void Board::setMode(GameMode mode)
{
    ...
    else if (mode == modeNormal && old_mode == modeEdit)
    {
        ...
        /* Normally, we add an edited board as a new child.  However, when the
           original position was the root node, or an edit node, and has no children
           yet, just update it in-place. It can be debated whether this behaviour
           would be surprising to users, but it seems like the most useful way to
           handle the situation.  */
        if (m_displayed->n_children() == 0 && (m_displayed->root_node_p() || m_displayed->was_edit_p()))
        {
...
/* Render a mark in SVG at center position CX/CY in a square with a side length
   of FACTOR. M and ME represent the mark as present in the board.  SC is the
   color of the stone that has been rendered before, or NONE.  COUNT_VAL, if
   nonzero, together with MAX_NUMBER represents the move number to be displayed.
   VAR_M and VAR_ME represent a variation mark, typically some letter.

   WHITE_BACKGROUND is true if we are doing this for svg export, it changes the
                         ...
                         const QFontInfo   &fi)
{
    ...
    case mark::circle:
        ...
        break;
    case mark::square:
        svg.square_at(cx, cy, factor * 0.6 / M_SQRT2, "none", mark_col);
        break;
    case mark::redbox:
        svg.square_at(cx, cy, factor * 0.25 / M_SQRT2, "red", "red");
        break;
    case mark::triangle:
        svg.triangle_at(cx, cy, factor * 0.6, "none", mark_col);
        break;
    case mark::terr:
        ...
        /* fall through */
    case mark::cross:
        svg.cross_at(cx, cy, factor * 0.6, mark_col);
        break;
    case mark::num:
...
QByteArray BoardView::render_svg(bool do_number, bool coords)
{
    ...
    /* A white background, since use white squares to clear the grid when showing
       marks. Against a clear background that wouldn't look very good.  */
    ...
    for (int tx = 0; tx < visx + 1; tx++)
    {
        ...
        if (m_crop.aligned_top(m_dims) && (tx == visx || x == 0 || grid_hidden.test_bit(bp1)))
        {
            if (yfirst1 != -2)
            {
                ...
                if (x == 0)
                    last--;
                painter.drawLine(line_offx + yfirst1 * square_size / 2,
                                 line_offy + ty1 * square_size,
                                 line_offx + last * square_size / 2,
                                 line_offy + ty1 * square_size);
            }
            yfirst1 = -2;
        ...
        if (m_crop.aligned_bottom(m_dims) && (tx == visx || x == 0 || grid_hidden.test_bit(bp2)))
        {
            if (yfirst2 != -2)
            {
                ...
                if (x == 0)
                    last--;
                painter.drawLine(line_offx + yfirst2 * square_size / 2,
                                 line_offy + ty2 * square_size,
                                 line_offx + last * square_size / 2,
                                 line_offy + ty2 * square_size);
            }
            yfirst2 = -2;
    ...
    for (int ty = 0; ty < visy + 1; ty++)
    {
        ...
        if (m_crop.aligned_left(m_dims) && (ty == visy || y == 0 || grid_hidden.test_bit(bp1)))
        {
            if (xfirst1 != -2)
            {
                ...
                if (y == 0)
                    last--;
                painter.drawLine(line_offx + tx1 * square_size,
                                 line_offy + xfirst1 * square_size / 2,
                                 line_offx + tx1 * square_size,
                                 line_offy + last * square_size / 2);
            }
            xfirst1 = -2;
        }
        if (m_crop.aligned_right(m_dims) && (ty == visy || y == 0 || grid_hidden.test_bit(bp2)))
        {
            if (xfirst2 != -2)
            {
                ...
                if (y == 0)
                    last--;
                painter.drawLine(line_offx + tx2 * square_size,
                                 line_offy + xfirst2 * square_size / 2,
                                 line_offx + tx2 * square_size,
                                 line_offy + last * square_size / 2);
            }
            xfirst2 = -2;
        }
    ...
    const go_board &b, const bit_array *visible, stone_color to_move, int x, int y, const go_board &vars, int var_type)
{
    ...
    /* If we don't have a real stone, check for various possibilities of
       ghost stones: the mouse cursor, then territory marks, then variation
       display.  */
    ...
    svg_builder &svg, double svg_factor, double cx, double cy, const QFontInfo &fi, int x, int y, bool child_mark, int v, int max_number)
{
    ...
    /* Put a percentage or letter mark on variations returned by the analysis
       engine, unless we are already showing a numbered variation and this
       intersection has a number.  */
...
/* The central function for synchronizing visual appearance with the abstract
 * board data.  */
QPixmap BoardView::draw_position(int default_vars_type)
{
    ...
    /* There are several ways we can get move numbering: showing a PV line from
       analysis, or when displaying a figure, or displaying numbers on a normal
       board.  The first and second are mutually exclusive, and have priority.  */
    int      max_number = 0;
    go_board mn_board(b, mark::none);
    ...
    /* Handle marks first.  They go into an svgbuilder which we'll render at the
       end, but we also use this to decide which parts of the grid to hide for
       better readability.  */
...
void Board::wheelEvent(QWheelEvent *e)
{
	...
	if (QTime::currentTime() < wheelTime)
		return;
#endif
    // Needs an extra check on variable mouseState as state() does not work on
    // Windows.
...
void Board::mouseReleaseEvent(QMouseEvent *e)
{
    ...
    // qDebug("Mouse should be released after %d,%03d",
    // wheelTime.second(),wheelTime.msec()); qDebug("Mouse released at time
    // %d,%03d", QTime::currentTime().second(),QTime::currentTime().msec());
...
void Board::click_add_mark(QMouseEvent *e, int x, int y)
{
    ...
    if (mark_to_set == mark::letter && e->modifiers() == Qt::ShiftModifier)
    {
        ...
        {
            /* This is all a bit clunky; it's the price to pay for having game_state's
               get_board return a const go_board.  */
            if (m_edit_board)
...
void Board::mousePressEvent(QMouseEvent *e)
{
    ...
    /* All modes where marks are not allowed, including scoring, force the
       editStone button instead of the marks.  So this is a simple test.  */
    if (m_edit_mark != mark::none)
    {
...
#if 0 /* We get our own toggle back from the server, at least in tests with \
         NNGS.  */
...
FigureView::FigureView(QWidget *parent) : BoardView(parent)
{
    m_figure_view = true;
}
