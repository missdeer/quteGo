    ...
    for (auto &p : paras)
    {
        ...
        for (auto &w : words)
        {
            ...
            else
            {
                ...
                {
                    painter.drawStaticText(margin_px, margin_px + lineno * fh_chosen, QStaticText(line));
                    line.clear();
                    lineno++;
                    if (lineno == n_lines)
                        break;
...
bool SlideView::save()
{
    ...
    if (pattern.isEmpty() || !pattern.contains("%n"))
    {
        QMessageBox::warning(this,
                             ...
                             tr("Please enter a filename pattern which includes "
                                "\"%n\" where the number should be substituted."));
        return false;
    }
    ...
    if (f.exists() && !overwriteCheckBox->isChecked())
    {
        ...
        choice = QMessageBox::warning(this,
                                      tr("File exists"),
                                      tr("A filename matching the pattern and current number already exists. "
                                         " Overwrite?"),
...
void SlideView::save_all(bool only_comments)
{
    ...
    std::function<bool(game_state *)> fn = [this, only_comments, &collection](game_state *st) -> bool {
