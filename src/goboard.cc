...
go_board::go_board(int w, int h, bool torus_h, bool torus_v)
    : m_sz_x(w),
      ...
      m_torus_h(torus_h),
      m_torus_v(torus_v),
      m_masked_left(create_boardmask_left(w, h)),
      m_masked_right(create_boardmask_right(w, h)),
      ...
      m_stones_b(new bit_array(w * h)),
      m_stones_w(new bit_array(w * h))
{
}
...
void go_board::scoring_flood_fill(bit_array &fill, const bit_array &w_stones, const bit_array &b_stones, bool &neighbours_w, bool &neighbours_b)
{
    bit_array next(fill);
    for (;;)
    {
...
void go_board::find_territory_units(const bit_array &w_stones, const bit_array &b_stones)
{
    ...
    for (unsigned i = 0; i < bitsize(); i++)
    {
        ...
        if (neighbours_w != neighbours_b)
            m_units_t.emplace_back(fill, neighbours_w, neighbours_b, fill.intersect_p(dead_stones));
        else
...
void go_board::benson(std::vector<stone_unit> &units, const bit_array &other_stones)
{
    ...
    for (;;)
    {
        ...
        tentative.erase(std::remove_if(std::begin(tentative),
                                       std::end(tentative),
                                       [&](size_t idx) {
        ...
        eas.erase(std::remove_if(std::begin(eas),
                                 std::end(eas),
                                 [&](enclosed_area &ea) {
...
void go_board::add_stone(int x, int y, stone_color col, bool process_captures)
{
...
#endif
        opponent_units.erase(
            std::remove_if(opponent_units.begin(), opponent_units.end(), [](const stone_unit &unit) { return unit.m_n_liberties == -1; }),
            opponent_units.end());
#ifdef CHECKING
    ...
    player_units.erase(std::remove_if(player_units.begin(), player_units.end(), [](const stone_unit &unit) { return unit.m_n_liberties == -1; }),
                       player_units.end());
