...
class GTP_Process : public QProcess
{
    ...
    /* A tree in which we keep track of what the move history, in sync with the
     * GTP engine.  */
    game_state *m_moves {};
...
public:
    GTP_Process(QWidget *parent, GTP_Controller *c, const Engine &engine, int size, float komi, bool show_dialog = true);
    ~GTP_Process();
    bool started()
    {
        return m_started;
