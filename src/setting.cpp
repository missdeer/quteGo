...
Setting::Setting()
{
    ...
    writeIntEntry("STONES_SIZE_PERCENT", 97);
    ...
    if (updated)
        QMessageBox::information(nullptr,
                                 ...
                                 QObject::tr("Engine configuration updated\n"
                                             "Analysis engines now require a board size to be set, "
                                             "assuming 19 for existing entries."));
}
...
void Setting::loadSettings()
{
    ...
    /* Can't write this as a default in the constructor: existing preferences may
       have a wood image for which scaling would be inappropriate.  So check here
       if we still have the default SKIN_INDEX and no SKIN_SCALE_WOOD after
       loading.  */
    if (!params.contains("SKIN_SCALE_WOOD"))
        writeBoolEntry("SKIN_SCALE_WOOD", readIntEntry("SKIN_INDEX") == 1);

    // Read obsolete font entries first, and then try to replace them with more
    // current ones.
...
QString Setting::getLanguage()
{
    ...
    if (l.isEmpty() || l == QString("Default"))
    {
        ...
        return l; //.latin1();
                  // return readEntry("LANG") == NULL ? getenv("LANG") :
                  // readEntry("LANG").latin1(); }
}
