...
InfoType Parser::put_line(const QString &txt)
{
    ...
    // correct cmd_nr for special case; if quiet is set to false a game may not
    // end...
    ...
    case 24:
        ...
        // results
        // 25 File
        // curio      [ 5d*](W) : lllgolll   [ 4d*](B) H 0 K  0.5 19x19 W+Resign
        // 22-04-47 R curio      [ 5d*](W) : was        [ 4d*](B) H 0 K  0.5 19x19
        // W+Time 22-05-06 R 25 File
    case 25:
        break;
    case 27:
        return cmd27(txt);
...
// 7 [41]      xxxx10 [ 1k*] vs.      xxxx12 [ 1k*] (295   19  0  5.5 12  I) (
// 1) 7 [118]      larske [ 7k*] vs.      T08811 [ 7k*] (208   19  0  5.5 10  I)
// (  0) 7 [255]          YK [ 7d*] vs.         SOJ [ 7d*] ( 56   19  0  5.5  4
// I) ( 18) 7 [42]    TetsuyaK [ 1k*] vs.       ezawa [ 1k*] ( 46   19  0  5.5
// 8  I) (  0) 7 [237]       s2884 [ 3d*] vs.         csc [ 2d*] (123   19  0
// 0.5  6  I) (  0) 7 [67]    atsukann [14k*] vs.    mitsuo45 [15k*] ( 99   19
// 0  0.5 10  I) (  0) 7 [261]      lbeach [ 3k*] vs.    yurisuke [ 3k*] (194 19
// 0  5.5  3  I) (  0) 7 [29]      ppmmuu [ 1k*] vs.       Natta [ 2k*] (141 19
// 0  0.5  2  I) (  0) 7 [105]      Clarky [ 2k*] vs.       gaosg [ 2k*] ( 65 19
// 0  5.5 10  I) (  1)
...
//	9 Channel 20 Topic: [xxx] don't pay your NIC bill and only get two
// players connected 	9  xxxx1 xxxx2 xxxx3 xxxx4 frosla 	9 Channel 49 Topic: Der
// deutsche Kanal (German) 	9  frosla
//
//	-->  channel 49
...
//	9 You can check your score with the score command, type 'done' when
// finished. 	9 Removing @ K8
//
...
//	9     -- -- kou         6k*   0   0 23s  NR
//	9 SQ! -- -- GnuGo      11k*   0   0  5m  Estimation based on NNGS rating
// early 	9   X -- -- Maurice     3k*   0   0 24s  2d at Hamilton Go Club, Canada;
// 3d in
InfoType Parser::cmd9(QString &line)
{
    ...
    // 9 Use <nmatch yfh2test B 3 19 60 600 25 0 0 0> or <decline yfh2test> to
    // respond.
    ...
    // 9 Verbose  Bell  Quiet  Shout  Automail  Open  Looking  Client  Kibitz
    // Chatter 9     Off    On     On     On        On   Off      Off      On On
    // On
    ...
    // 9 ROOM 01: FREE GAME
    // ROOM;PSMNYACF;���R�΋ǎ�;19;1;10;1,19,60,600,30,25,10,60,0 9 ROOM 10: SLOW
    // GAME ROOM;PSMNYACF;�۰�΋ǎ�;19;1;20 9 ROOM 92: PANDA OPEN
    // ROOM;X;������;19;10;15
...
//  24 *xxxx*: CLIENT: <cgoban 1.9.12> match xxxx wants handicap 0, komi 5.5,
//  free 24 *jirong*: CLIENT: <cgoban 1.9.12> match jirong wants handicap 0,
//  komi 0.5 24 *SYSTEM*: shoei canceled the nmatch request. 24 *SYSTEM*: xxx
//  requests .

// NNGS - new version:
//  24 --> frosla CLIENT: <qGo 0.0.15b7> match frosla wants handicap 0, komi
//  0.5, free 24 --> frosla  Hallo
...
// 7 [255]          YK [ 7d*] vs.         SOJ [ 7d*] ( 56   19  0  5.5  4  I) (
// 18) 42 [88]          AQ [ 2d*] vs.        lang [ 2d*] (177   19  0  5.5  6 I)
// (  1)

// IGS: cmd 'user'
// 42 Name        Info            Country  Rank Won/Lost Obs  Pl Idle Flags
// Language 0  3           15              31       40   45 48    54   59 62 67
// 72 42    guest15                  --        NR    0/   0  -   -   10s    Q-
// default 42    guest13                  --        NR    0/   0  -   -   24s Q-
// default 42         zz  You may be rec  France    NR    0/   0  -   -    0s -X
// default 42     zz0002  <none>          Japan     NR    0/   0  -   -    1m QX
// default 42     zz0003  <none> ()       Japan     NR    0/   0  -   -    1m SX
// default 42       anko                  Taiwan    2k* 168/  97  -   -    1s --
// default 42     MUKO12  1/12 〜 1/15    Japan     4k* 666/ 372  17  -    1m
// -X default 42        mof  [Igc2000 1.1]   Sweden    2k* 509/ 463 124  -    0s
// -X default 42     obiyan  <None>          Japan     3k* 1018/ 850  -   50 11s
// -- default 42    uzumaki                  COM       1k    0/   0  -   -    1s
// -X default 42 HansWerner  [Igc2000 1.2]   Germany   1k   11/   3 111  -    1m
// QX default 42    genesis                  Germany   1k* 592/ 409  -  136  14s
// -- default 42    hamburg                  Germany   3k* 279/ 259  -  334  13s
// -- default 42        Sie                  Germany   1k*   6/   7  -   68  39s
// -- default 42     borkum                  Germany   4k* 163/ 228  -  100   7s
// -- default 42     casumy  45-3            Germany   1d    0/   0  -  133   2s
// Q- default 42      stoni                  Germany   2k* 482/ 524  -  166   7s
// -- default 42   Beholder  <None>          Germany   NR    0/   0 263  -    5s
// QX default 42     xiejun  1/10-15         Germany   3d* 485/ 414 179  -   49s
// -X default 9                 ******** 1120 Players 247 Total Games ********
...
// 42 Name        Info            Country  Rank Won/Lost Obs  Pl Idle Flags
// Language 42      -----  <none>          Japan    15k+ 2108/2455  -   -    1m
// Q- default  T BWN 0-9 19-19 600-600 1200-1200 25-25 0-0 0-0 0-0 42      -----
// Japan     3d   11/  13  -  222  46s    Q- default  T BWN 0-9 19-19 60-60
// 600-600 25-25 0-0 0-0 0-0 42       Neil  <None>          USA      12k  136/
// 86  -   -    0s    -X default  T BWN 0-9 19-19 60-60 60-3600 25-25 0-0 0-0
// 0-0
