...
AnalyzeDialog::AnalyzeDialog(QWidget *parent, const QString &filename) : QMainWindow(parent), GTP_Eval_Controller(parent)
{
    ...
    /* "Swap komi if better" seems like a reasonable default.  It only affects
       games with negative komi.  */
...
{
    /* We could allow the user to hide this dialog and let things run in the
       background. The downside of that is that this may be the only window and
       we'd have to check for that case.  Also, I think it's good to have the
       window open as a reminder that an engine is running.  */
    if (m_jobs.jobs.size() != 0)
    {
        QMessageBox::StandardButton choice;
        choice = QMessageBox::warning(this,
                                      PACKAGE,
                                      tr("Jobs are still running.  Do you wish to terminate the engine and "
                                         "discard the jobs?"),
    ...
    else if (m_done.jobs.size() != 0)
    {
        ...
        choice = QMessageBox::warning(this,
                                      PACKAGE,
                                      tr("Completed jobs contain unsaved data.  Do "
                                         "you wish to discard the jobs?"),
                                      QMessageBox::Yes | QMessageBox::Cancel,
                                      QMessageBox::Yes);
...
AnalyzeDialog::job::job(AnalyzeDialog *dlg, QString &title, go_game_ptr gr, int n_seconds, int n_lines, engine_komi k, bool comments)
    : m_dlg(dlg), m_title(title), m_game(gr), m_n_seconds(n_seconds), m_n_lines(n_lines), m_komi_type(k), m_comments(comments)
{
    ...
    std::function<bool(game_state *)> f = [this, &q](game_state *st) -> bool {
        ...
        if (st->has_figure() && ev.visits > 0)
            return false;
        /* Ignore score and pass nodes on the grounds that they should be identical
...
void AnalyzeDialog::eval_received(const QString &, int, bool have_score)
{
    ...
    if (j == nullptr)
    {
        /* This occurs when the currently processed job is manually deleted by the
         * user.  */
        queue_next();
        return;
    }
