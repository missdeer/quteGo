...
class go_board
{
    ...
    /* An anclosed area for a color C1 is obtained by a flood fill through empty
       spaces and stones of the opposite color C2.  We also obtain the border of
       the area, by expanding once in each direction and then removing the actual
       area (note that the border can contain stones in the interior).  Finally,
       before storing the area, stones of C2 are removed from the bit set.  */
    ...
    /* Number of captures that have occurred on the board.  Note: these indicate
       the captures made _by_ a side, so m_caps_b is the number of white stones
       captured.  */
    int m_caps_b = 0;
    int m_caps_w = 0;
    /* Number of stones marked as captured during the scoring phase.  Here,
       m_dead_b is the number of dead black stones.  */
    int        m_dead_b = 0;
    int        m_dead_w = 0;
    ...
    /* For mark::text, the mark_extra field is an index into this table of
     * strings.  */
...
public:
    ...
    go_board(const go_board &other)
        : m_sz_x(other.m_sz_x),
          ...
          m_stones_b(new bit_array(*other.m_stones_b)),
          m_stones_w(new bit_array(*other.m_stones_w)),
    ...
    /* The unused mark argument should be passed as mark::none by the callers to
       indicate what this constructor is for: copying a board position without
       copying marks. Note that this implies territory markers are not copied, and
       we do not copy m_dead_b and m_dead_w which just summarize that information.
     */
    ...
    /* A similar constructor to create an empty board with identical properties to
       another. Callers should pass none to the unused stone_color argument for
       clarity.  */
    go_board(const go_board &other, stone_color)
        : m_sz_x(other.m_sz_x),
          ...
          m_row_top(other.m_row_top),
          m_row_bottom(other.m_row_bottom),
          m_stones_b(new bit_array(other.bitsize())),
          m_stones_w(new bit_array(other.bitsize()))
    {
    }
    ...
    bool set_mark(int x, int y, mark m, mextra extra)
    {
        init_marks(false);
        int bp = bitpos(x, y);
        if ((size_t)bp >= m_marks.size() || (size_t)bp >= m_mark_extra.size())
            return false;
